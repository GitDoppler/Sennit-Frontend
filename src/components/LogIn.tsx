...
export default function LogIn({signup,setSignup}:{signup:boolean,setSignup:Dispatch<SetStateAction<boolean>>}){
    ...
    return(
        <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
            <form onSubmit={handleSubmit} className="p-8 rounded-2xl bg-[#121212] flex flex-col gap-12">
                <div>
                    <h2 className=" text-2xl font-bold text-white pb-4">Log In</h2>
                    <p className="text-[#B6B6B6]">New to Sennit ? <button onClick={()=>setSignup(!signup)} className="text-[#00DC82]">Sign up for an account</button></p>
                </div>
